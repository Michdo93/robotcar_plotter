...
class FrontRightUltrasonicSubscriber(object):
    ...
    def __init__(self, robot_host):
        ...
        self.sub = rospy.Subscriber(self.robot_host + '/ultrasonic/front/right/distance', Range, self.callback)
    ...
    def callback(self, data):
        ...
        plt.title('FRONT RIGHT ULTRASONIC SENSOR - ENVIRONMENT MAP', fontname='monospace', color='black', fontsize=16)

        p1, = plt.plot(self.x0, self.y0, color='b')
        plt.legend([p1], ['ULTRASONIC Reading'], prop={'family': 'monospace'}, loc='upper right', frameon=True)
...
if __name__ == '__main__':
    ...
    node_name = re.sub("-", "_", socket.gethostname()) + "_FrontRightUltrasonicSubscriber"
    rospy.init_node(node_name, anonymous=False)
    
    ultrasonic = FrontRightUltrasonicSubscriber("robotcar")
